...
void main() {
    ...
    await tester.pumpWidget(Localizations(
      ...
      child: Directionality(
        ...
        child: MediaQuery(
          data: topPadding100,
          child: Scaffold(
              primary: true, appBar: AppBar(title: const Text('title'))),
        ),
      ),
    ));
    expect(appBarTop(tester), 0.0);
  ...
  testWidgets(
    ...
    expect(tester.getRect(find.byType(AppBar)),
        Rect.fromLTRB(0.0, 0.0, 800.00, 100.0 + 56.0));
    expect(tester.getRect(find.byKey(leadingKey)),
        Rect.fromLTRB(800.0 - 56.0, 100.0, 800.0, 100.0 + 56.0));
    expect(tester.getRect(find.byKey(trailingKey)),
        Rect.fromLTRB(0.0, 100.0, 400.0, 100.0 + 56.0));
  });
