...
class ChangeNotifierInherited extends InheritedNotifier<ChangeNotifier> {
  const ChangeNotifierInherited(
      {Key key, Widget child, ChangeNotifier notifier})
...
void main() {
  testWidgets('Inherited notifies dependents', (WidgetTester tester) async {
    ...
    final TestInherited second =
        TestInherited(child: builder, shouldNotify: false);
    await tester.pumpWidget(second);

    expect(log, equals(<TestInherited>[first]));

    final TestInherited third =
        TestInherited(child: builder, shouldNotify: true);
                        ...
                        child: ValueInherited(
                            ...
                            child: Container(child: Container(
                              ...
                              final ValueInherited v = context
                                  .inheritFromWidgetOfExactType(ValueInherited);
                              log.add('b: ${v.value}');
                              return const Text('',
                                  textDirection: TextDirection.ltr);
                            }))))))))));
  ...
  testWidgets(
    ...
    final Widget child = Builder(builder: (BuildContext context) {
      final ValueInherited v =
          context.inheritFromWidgetOfExactType(ValueInherited);
      log.add(v.value);
  ...
  testWidgets(
    ...
    await tester.pumpWidget(ValueInherited(
        value: 2,
        child: FlipWidget(
            left: ValueInherited(value: 3, child: child), right: child)));
