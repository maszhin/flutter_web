...
void main() {
  testWidgets('Control test for custom painting', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(new CustomPaint(
        painter: new TestCustomPainter(log: log, name: 'background'),
        foregroundPainter: new TestCustomPainter(log: log, name: 'foreground'),
        child: new CustomPaint(
            painter: new TestCustomPainter(log: log, name: 'child'))));
    // TODO(het): enable once paint override gets called.
  ...
  testWidgets('CustomPaint sizing', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(new Center(child: new CustomPaint(key: target)));
    ...
    await tester.pumpWidget(new Center(
        child: new CustomPaint(
            key: target, size: Size.zero, child: new Container())));
    expect(target.currentContext.size, const Size(800.0, 600.0));

    await tester.pumpWidget(new Center(
        child: new CustomPaint(
            key: target, child: new Container(height: 0.0, width: 0.0))));
    expect(target.currentContext.size, Size.zero);
  });
