...
Future<void> main() async {
  ...
  group('FadeInImage', () {
    testWidgets('animates uncached image and shows cached image immediately',
      ...
      expect(displayedImage().image,
          null); // image providers haven't completed yet
      placeholderProvider.complete();
      await tester.pump();

      expect(displayedImage().image,
          same(placeholderImage)); // placeholder completed
      ...
      expect(stateAfterDidUpdateWidget.phase,
          FadeInImagePhase.completed); // completes immediately
      expect(displayedImage().image, same(targetImage));

      // Test case: new state object but cached image
      final dynamic stateBeforeRecreate = state();
      await tester
          .pumpWidget(Container()); // clear widget tree to prevent state reuse
      await tester.pumpWidget(FadeInImage(
        placeholder: placeholderProvider,
        image: imageProvider,
      ));
      ...
      expect(stateAfterRecreate.phase,
          FadeInImagePhase.completed); // completes immediately
      expect(displayedImage().image, same(targetImage));
    });

    testWidgets('handles a updating the placeholder image',
        (WidgetTester tester) async {
      RawImage displayedImage() => tester.widget(find.byType(RawImage));
      ...
      final TestImageProvider secondPlaceholderProvider =
          TestImageProvider(secondPlaceholderImage);
      ...
      expect(displayedImage().image,
          isNot(same(placeholderImage))); // placeholder replaced
