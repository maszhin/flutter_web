...
void main() {
  ...
  testWidgets('First frame decoding starts when codec is ready',
  ...
  testWidgets('ImageStream emits frame (static image)',
    ...
    final FrameInfo frame =
        new FakeFrameInfo(20, 10, const Duration(milliseconds: 200));
    mockCodec.completeNextFrame(frame);
    await tester.idle();

    expect(
        emittedImages, equals(<ImageInfo>[new ImageInfo(image: frame.image)]));
  });
  ...
  testWidgets('ImageStream emits frames (animated images)',
    ...
    await tester.pump();
    expect(
        emittedImages, equals(<ImageInfo>[new ImageInfo(image: frame1.image)]));

    final FrameInfo frame2 =
        new FakeFrameInfo(200, 100, const Duration(milliseconds: 400));
  ...
  testWidgets('frames are only decoded when there are active listeners',
      (WidgetTester tester) async {
    final MockCodec mockCodec = new MockCodec();
    mockCodec.frameCount = 2;
    mockCodec.repetitionCount = -1;
  ...
  testWidgets('multiple stream listeners', (WidgetTester tester) async {
    ...
    await tester.idle(); // let nextFrameFuture complete
    await tester.pump(); // first animation frame shows on first app frame.
    expect(emittedImages1,
        equals(<ImageInfo>[new ImageInfo(image: frame1.image)]));
    expect(emittedImages2,
        equals(<ImageInfo>[new ImageInfo(image: frame1.image)]));
