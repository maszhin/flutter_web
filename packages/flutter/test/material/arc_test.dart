...
void main() {
  test('MaterialPointArcTween control test', () {
    final MaterialPointArcTween a =
        MaterialPointArcTween(begin: Offset.zero, end: const Offset(0.0, 10.0));

    final MaterialPointArcTween b =
        MaterialPointArcTween(begin: Offset.zero, end: const Offset(0.0, 10.0));
  ...
  test('on-axis MaterialPointArcTween', () {
    ...
    tween =
        MaterialPointArcTween(begin: Offset.zero, end: const Offset(10.0, 0.0));
  ...
  test('MaterialRectArcTween', () {
    ...
    expect(tween.lerp(0.0), begin);
    expect(
        sameRect(tween.lerp(0.25), Rect.fromLTRB(120.0, 113.0, 259.0, 237.0)),
        isTrue);
    expect(sameRect(tween.lerp(0.75), Rect.fromLTRB(42.3, 206.5, 153.5, 354.7)),
        isTrue);
    expect(tween.lerp(1.0), end);

    tween = MaterialRectArcTween(begin: end, end: begin);
    expect(tween.lerp(0.0), end);
    expect(sameRect(tween.lerp(0.25), Rect.fromLTRB(92.0, 262.0, 203.0, 388.0)),
        isTrue);
    expect(
        sameRect(tween.lerp(0.75), Rect.fromLTRB(169.7, 168.5, 308.5, 270.3)),
        isTrue);
    expect(tween.lerp(1.0), begin);
  });
}
